...
def test_match_jds_returns_matches_for_existing_card(client):
...
def test_match_jds_404_for_missing_card(client):
    with patch("src.api.routers.jds.vectorstore.build_jd_index"):
        response = client.get("/api/jds/match?card_id=9999")
    assert response.status_code == 404


def test_match_jds_returns_empty_list_when_card_has_no_skill_tags(client):
    card_id = db.save_card(
        None,
...
def test_match_jds_builds_index_only_once_per_process(client):
