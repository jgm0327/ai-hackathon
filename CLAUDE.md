...
- [ ] `src/frontend/` — **아직 삭제하지 않음.** 9/15 게이트(P0 미완료 시 Streamlit으로
      마감)가 있는 동안은 폴백으로 남겨둬야 한다. 게이트를 통과해 Next.js로 확정된
      뒤에만 폐기할 것 — iframe 우회 코드(`install_button.py` 등)는 이식하지 않는다
- [ ] `manifest.json`, `README.md` — 제품명 갱신. **9/18로 미뤄도 됨** (CLAUDE.md 본문에
      명시된 우선순위 — P0보다 우선하지 않음)
- [ ] `requirements.txt` — streamlit 제거, fastapi/uvicorn 추가. **Next.js 전환이
      실제로 확정된 뒤에만** (위 src/frontend 항목과 동일한 이유로 보류)
