...
https://www.figma.com/design/MbQqcE5hGWI5KXwT3OtvIb/Connecting-Dots
  node-id=10-2   기획 정리 페이지 (화면 흐름, 타임라인, 페르소나)
  node-id=41-10  화면 목업 (2.0 온보딩 ~ 4.2 빌더)
```

### 3.1 에이전트가 이 파일을 읽으려면

- **Figma MCP 커넥터가 연결돼 있어야 한다.** 링크만으로는 접근 불가
- **편집자(Editor) 권한이 필요하다.** 뷰어 권한으로는 노드 조회가 거부된다
  (`get_design_context`, `get_metadata` 모두 "edit access" 오류)
- 주로 쓰는 도구: `get_design_context`(레이아웃·스타일), `get_screenshot`(시각 확인),
  `get_variable_defs`(색·간격 토큰)

**MCP가 없으면 이 문서의 3.3 화면 대응표와 Figma 스크린샷으로 작업한다.**
링크를 열려고 시도하다 시간을 쓰지 말 것.

### 3.2 Figma에 없는 것 — 이 문서가 우선한다

Figma 코멘트 핀에 달린 논의 내용은 **어떤 도구로도 읽을 수 없다**
(Figma Plugin API에 comment 접근이 존재하지 않음). 코멘트에서 나온 결정은
전부 `CLAUDE.md`와 이 문서에 글로 옮겨져 있다.

> **2026-09-18 정정**: 아래 "문서가 우선" 규칙은 **2026-09-16에 뒤집혔다** —
> 사용자 지시로 지금은 **Figma가 단일 진실 공급원**이고 문서가 Figma를 따라간다
> (CLAUDE.md 3.5장). 단, CLAUDE.md 2.4 배제 목록 5개는 그 뒤집힘과 무관하게 유지되며,
> Figma에 해당 화면이 있어도 구현 전에 사용자에게 먼저 확인한다. 아래 표는 그 뒤집힘
> 이전에 정리된 것이라 기록으로만 남겨둔다.

따라서 **Figma와 문서가 충돌하면 문서가 우선**한다. 특히:

| 항목 | Figma | 실제 |
|---|---|---|
| 프로젝트 스위처 | 없음 | **필수** (3.4) |
| 현재 직무 입력 | 없음 | 온보딩에 추가 (P2) |
| JD 붙여넣기 | 없음 | `/resume` 상단에 추가 |
| 숫자 되묻기 | 없음 | 결과 모달에 칩 (P2) |
| `4.2`의 프로젝트 헤드 | 기간 포함 | 프로젝트 층이 있어야 성립 |
| 음성 입력 UX | 미정 | **9/13 검증 결과에 따라 결정** (CLAUDE.md 8장) |

`4.2 빌더`가 "결제 코어 API 성능 개선 / 2023.04 - 2024.01"처럼 **기간 있는 프로젝트 단위**로
그려져 있는데, Figma의 입력·스택 화면에는 그 묶음을 만드는 장치가 없다.
프로젝트 층(CLAUDE.md 3장)이 그 구멍을 메운다.
...
| 4.1-b 기록 상세 | `/stack/<id>` | 주제별 타임라인 + 이어 쓰기 (9/18 신규) |
| 4.2.1 범위 선택 | `/resume` 상단 | 프로젝트·미분류 체크박스 (9/18 신규) |
| 4.2 빌더 | `/resume` | JD 붙여넣기, STAR 항목, 내보내기 |
| 5.0 설정 | `/settings` | 직무·알림·노션·백업·앱 정보 |

> `/projects`는 **9/16에 삭제됐다** — 프로젝트 전환/생성은 `<ProjectSwitcher />`로
> `/`·`/stack`에 인라인 통합됐다(Figma `89:525`, CLAUDE.md 3.5장).

### 3.4 프로젝트 스위처 (신규, Figma에 없음)
...
- [x] **4.2.1 범위 선택** (9/18, Figma `89:161`) — `web/components/ResumeScopePicker.tsx`.
      `/resume` 진입 시 "그동안 이런 걸 하셨어요"(기록/프로젝트/기록한 날 집계) +
      프로젝트·미분류 체크박스로 초안 범위를 고른다. **기본값은 현재 프로젝트 하나**라
      아무것도 안 건드리고 버튼만 누르면 9/18 이전과 동일하게 동작한다. 여러 개를
      고르면 결과에 프로젝트 헤드(`41:254`, 이름+프로젝트 기간)가 붙고 마크다운/Word
      내보내기도 `## 프로젝트` + `### 항목` 2단으로 나뉜다. 캐시/직접수정 override
      키를 프로젝트 id에서 "범위 키"로 바꿨다(`resumeScopeKey`) — 단일 프로젝트 범위의
      키는 예전과 같은 문자열이라 저장돼 있던 값이 그대로 이어진다
- [x] **4.1-b 기록 상세** (9/18, Figma `89:479`) — `web/app/stack/[id]/page.tsx` 신규.
      `/stack`에서 카드 문장을 누르면 진입. 같은 프로젝트 + 같은 대표 태그를 한 "주제"로
      보고 그 주제의 기록을 시간순으로 보여준다. 이 묶음은 **저장하지 않는다**(매번
      카드에서 다시 계산, CLAUDE.md 3장) — LLM도 안 부른다. "이 주제에 이어 쓰기"는
      `/?topic=<태그>`로 홈에 돌아가고, 저장 직후 그 태그를 카드에 얹어 같은 주제로
      묶이게 한다(유저가 직접 고른 값이라 추측이 아니다).
      **Figma와 다른 점**: Figma는 맨 위에 여러 기록이 합쳐진 "통합 경력 문장"을
      보여주는데, 우리 모델에서 문장은 카드 한 장에 하나씩이고 합치는 건 초안 생성
      시점의 일이다. 없는 문장을 여기서 새로 만들면 화면을 열 때마다 LLM을 부르게 되고
      실제 경력기술서와 다른 문장이 나온다 — 그래서 카드의 실제 문장을 보여주고
      "초안 생성 때 합쳐진다"고 안내하는 쪽을 택했다
- [x] **`/` 이어 쓰기 배너 / 타깃 트랙 칩 / 노션 링크** (9/18, Figma `41:119`/`41:116`/
      `41:128`) — 배너는 "오늘 이전 마지막 기록의 대표 태그"를 쓰고, 이미 그 주제를
      오늘 건드렸으면 뜨지 않는다. 타깃 트랙은 프로필이 비어 있으면 아예 안 그린다
- [x] **복사 완료 토스트** (9/18, Figma `41:880`) — `web/components/Toast.tsx`.
      `/`(결과 시트)·`/stack/<id>`·`/settings`에서 재사용
- [x] **설정 백업 내보내기/불러오기** (9/18, Figma `41:333`/`41:338`) — `GET /api/backup`
      으로 JSON을 받아 파일로 내려받고, 파일을 고르면 확인 시트를 거쳐
      `POST /api/backup/import`로 보낸다. **덧붙이기라 기존 기록을 지우지 않는다** —
      누르기 전에 그 사실을 문구로 알린다
...
## 9. Figma 리스타일링 (9/14, 4번째 세션)

전달받은 Figma 레퍼런스(색/간격/라운드/타이포)를 4개 화면(`/`, `/stack`, `/resume`,
결과 모달)에 적용했다. **로직/상태/API 호출은 그대로 두고 시각만 다시 입힌 세션** —
Figma 목업의 예시 텍스트("24", "연속 3일", "결제 서버 API 응답 지연으로…" 등)는
전부 플레이스홀더이므로 어디에도 하드코딩하지 않았다(CLAUDE.md 2.2 정신).

### 9.1 적용 내역
- 공통 토큰: 배경 `#f1f1f3`(`app/layout.tsx`), 카드 `white`/`border-[#e5e7eb]`,
  1차 버튼 `bg-black rounded-[14px]`, 칩 `rounded-[999px]` 계열을 4개 화면에 일괄 적용
- `/` — 헤더 "오늘의 기록" + 원형 `⚙` 칩(`/onboarding`행). **신규**: "타깃 트랙" 행
  (`GET /api/profile` 기반, 프로필 없으면 "직군 설정하기"로 대체 — 값을 지어내지
  않음). 텍스트 입력창 + 마이크를 하나의 카드로 합침(`VoiceInput`에 `variant="icon"`
  추가, 기존 로직 그대로 재사용). **신규**: "노션에서 가져오기" — 그동안 백엔드
  (`POST /api/notion/sync`)만 있고 프론트가 없었던 기능 갭이라 최소 버전(토큰 입력 +
  동기화, OAuth/페이지 선택 없음)으로 구현(`lib/api.ts`의 `syncNotion()`)
- 결과 표시를 인라인 카드 → `BottomSheet` 모달로 전환(Figma "3.1"). 복사(`복사됨` 토스트)
  / "다시 변환"(시트 닫고 포커스 복귀) 액션 포함. `BottomSheet`에 `panelClassName`/
  `hideHandle` optional prop을 추가해 재사용 폭을 넓힘(기존 호출부인
  `ProjectSwitcher`는 기본값 그대로라 영향 없음)
- `/stack` — 헤더에 실제 카드 개수 노출, 태그 필터 칩 pill 스타일 재적용, 카드 리스트
  재스타일. **신규(작게)**: `⋯` → 인라인 삭제 확인(`DELETE /api/cards/{id}` 연결).
  하단 CTA를 `/resume`로 가는 링크로 변경
- `/resume` — 가장 큰 구조 변경. 항목별 카드(`StarItemCard`) 나열 → 하나의 연속
  문서(제목 + 항목별 상황/과제/행동/결과 라벨 행, 구분선)로 재구성
  (`components/StarItemCard.tsx`의 `StarItemCard` → `StarItemSection`으로 교체).
  문서 제목은 온보딩 프로필/현재 프로젝트명 중 있는 값만 조합 — `build_resume()`이
  만들지 않는 요약 문단은 생성하지 않음(2.2). 내보내기 3버튼: "마크다운"/"노션 복사"는
  클립보드 복사(같은 마크다운 텍스트, 라벨만 구분), "Word"는 `disabled` + "준비 중"
  표시(실제 .docx 생성은 스코프 밖). 기존 JD 붙여넣기 폼은 유지하되 결과 생성 후에는
  접어두고(`JD를 바꿔 다시 만들기` 토글) 문서 뷰가 기본으로 보이게 함

### 9.2 명시적으로 스킵/보류한 것
- **주간 기록 스트릭 위젯** (`/stack`) — 스트릭을 추적할 데이터 모델이 없음(카드에
  연속일 개념 없음). 지어내지 않기 위해 통째로 제외
- **이어 쓰기 배너** ("어제 하던 [Redis 캐싱] 이어 쓰기", `/` 상단) — "진행 중인
  작업 이어쓰기" 개념이 현재 데이터 모델에 없음. 가짜로 만들지 않고 제외
- **"기록은 이 기기 안에만 저장됩니다" 캡션** (`/stack`) — 실제로는 서버 SQLite에
  저장되므로 사실과 다름. 캡션 자체를 뺐다
- **생성 일자/글자수 메타 라인** (`/resume` 헤더) — 생성 시각을 API가 안 주므로
  지어내지 않음. 글자수는 계산 가능했지만 큰 가치가 없다고 판단해 통째로 뺌
- **Word(.docx) 실제 내보내기** — 실제 문서 생성 라이브러리 연동은 이번 세션 스코프
  밖. 버튼은 눈에 보이게 `disabled` + "준비 중"으로 정직하게 표시만 함
- **`/stack` 카드 액션 시트 고도화** — Figma의 "4.1-a 카드 액션 시트"(별도 바텀시트)
  대신 `⋯` 탭 시 같은 줄에서 "삭제/취소"만 인라인으로 보여주는 더 가벼운 버전으로
  구현. 시간 대비 스코프상 의도적으로 축소함 — 필요시 `BottomSheet` 기반 시트로
  교체 가능
- Figma의 iOS 상태바 목업("9:41" 시계/신호 바)은 렌더링하지 않음(디자인 파일의
  시뮬레이터 프레임일 뿐, 실제 화면 요소 아님)

### 9.3 검증
- `npm run build`, `npm run lint` 모두 0 에러
- `npm run dev` 기동 후 `/`, `/stack`, `/resume` curl로 200 확인 + 신규 마크업
  ("타깃 트랙"/"노션에서 가져오기", 카드 개수/필터/CTA, JD 폼/문서 뷰) 포함 확인.
  백엔드 미기동 상태라 실제 API 응답을 받은 렌더링(프로필 값 채워진 "타깃 트랙",
  결과 모달 실제 노출, STAR 문서 실제 렌더링)은 **여전히 실기기/실 백엔드 확인 필요**
  — 이번 세션에서 못 함(브라우저 자동화 도구 없음)
